...
- **React Flow state synchronization for editable fields (2025-01-27)**: When implementing editable fields in the Workflow Designer property panel, the input values were reading from `initialNodes` (props) but updating `flowNodes` (React Flow state). This caused a mismatch where typing wouldn't update the displayed value. **Lesson**: For controlled inputs in React Flow components, always read from the current React Flow state (`selectedNode?.data`) first, with fallback to initial props. Ensure `onChange` handlers update the React Flow state and trigger auto-save. Use `selectedNodeId` consistently instead of `selectedNode.id` to avoid potential null reference issues.

- **Database schema migrations for existing tables (2025-01-27)**: When adding new columns to existing database tables, `CREATE TABLE IF NOT EXISTS` won't modify existing tables. Need to add explicit migration logic that checks for column existence using `PRAGMA table_info()` and uses `ALTER TABLE` to add missing columns. **Lesson**: Always include migration checks when schema changes are made, especially for columns that might be added after initial table creation. Use try-catch blocks to handle cases where tables might not exist yet.
